...
def index():
    ...
    if from_number in callers:
        ...
        caller = "Persona desconocida "

    response = twilio.twiml.Response()
    response.say("Hola " + caller, language="es")    # Greet the caller by name
    response.play("http://demo.twilio.com/hellomonkey/monkey.mp3")     # Play an mp3
    ...
    if knownCaller == True:
    	...
    	with response.gather(numDigits=1, action="/api/handle-key", method="POST") as g:
    		g.say("Para hablar con Gonzalo, presiona 1.", language="es")
...
def main():
    ...
    try:
        ...
        logging.info(' * Starting dialer server' + ' version: ' + config.get('System', 'version'))
        print ' * Starting dialer server' + ' version: ' + config.get('System', 'version')
